...
answer_key = r[idx][0].question.correct_number
...
for each_group in group:
    v1.append([r[idx].filter(student__section__section_no=section_no, student__group__group_no=each_group, vote1=c).count() for c in choice])
    v2.append([r[idx].filter(student__section__section_no=section_no, student__group__group_no=each_group, vote2=c).count() for c in choice])
...
fig, axes = plt.subplots(2, no_groups_section - 1, sharex=True, sharey=True, figsize=(32,4))
